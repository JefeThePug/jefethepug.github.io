...
function plinkoGame(p) {
    ...
    p.preload = () => {
        for (let i = 1; i < 9; i++) {
            dings[i] = p.loadSound("assets/" + i + ".wav");
        }
        ball = p.loadImage("assets/ball.png");
        board = p.loadImage("assets/bg.jpg");
        for (let sb = 0; sb < 6; sb++) {
            scoreballs[sb] = p.loadImage("assets/plink" + sb + ".png");
        }
    }
    ...
    p.setup = () => {
        ...
        //borders
        let b1 = new Boundary(p.width / 2, p.height + 50, p.width, 100, "floor");
        bounds.push(b1);
        let b2 = new Boundary(-45, p.height / 2, 100, p.height, "side");
        bounds.push(b2);
        let b3 = new Boundary(p.width + 45, p.height / 2, 100, p.height, "side");
        bounds.push(b3);
        //buckets
    ...
    p.keyPressed = () => {
        if (p.key == " " && particles.length < 5) {
            ...
            giveBonus();
        } else if (p.keyCode == p.SHIFT && particles.length > 0) {
            tilt();
        }
    }
...
function letterGame(p) {
    ...
    let BGM, guessMusic, sfChime; //var sfLetterPress;

    p.preload = () => {
        for (let i = 0; i < ref.length; i++) {
            constellationImages[i] = p.loadImage("assets/" + ref[i] + ".jpg");
        }
        for (let j = 0; j <= 6; j++) {
            scoreStarImages[j] = p.loadImage("assets/" + j + ".png");
        }
        splash = p.loadImage("assets/_title.jpg");
        instruc = p.loadImage("assets/_instruc.jpg");
        messageImages[0] = p.loadImage("assets/_tryagain.jpg");
        messageImages[1] = p.loadImage("assets/_welldone.jpg");
        musicnote = p.loadImage("assets/_music.png");
        fontStars = p.loadFont("assets/_ABCBULLE.TTF");
        fontShown = p.loadFont("assets/_ABCPRINT.TTF");
        BGM = p.loadSound("assets/_music/bgm.mp3");
        guessMusic = p.loadSound("assets/_music/guess.mp3");
        sfChime = p.loadSound("assets/_music/sfChime.mp3");
    }
...
function phonicsGame(p) {
    ...
    p.preload = () => {
        ...
        bg = p.loadImage('assets/bg_p.jpg');
        logo = p.loadImage('assets/logo.jpg');
        instruc = p.loadImage('assets/instruc.jpg');
        highscorebg = p.loadImage('assets/highscore.jpg');
        for (let i = 0; i < ref.length; i++) {
            characterSet[i] = p.loadImage('assets/' + ref[i] + ".png");
        }
        BGM_p = p.loadSound('assets/sounds/BGM.mp3');
        sfCharClear = p.loadSound('assets/sounds/CharClear.wav');
        sfGameOver = p.loadSound('assets/sounds/GameOver.wav');
        sfLetterPress = p.loadSound('assets/sounds/LetterPress.wav');
    }
    ...
    p.keyPressed = () => {
        if (p.keyCode == p.RIGHT_ARROW) {
            ...
            blaster.setRotation(-0.02);
        } else if (p.keyCode > 64 && p.keyCode < 91) {
            textBlocks.unHighlight();
            blaster.firing(true, p.key, p.keyCode);
            let keyspot = textBlocks.outputLog().indexOf(p.key.toLowerCase());
            if (keyspot >= 0) {
    ...
    p.keyReleased = () => {
        ...
        } else if ((p.keyCode > 64 && p.keyCode < 91) || p.key == ' ') {
